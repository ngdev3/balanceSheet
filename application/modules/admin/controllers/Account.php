...
class Account extends CI_Controller {
	...
	public function billingCyle(){
		echo $this->Account_mod->checkRandomEntery();
	}
	...
	function edit($id = null){
        ...
        if (isPostBack()) {
            ...
            if ($this->form_validation->run() == FALSE) {
				...
				$this->Account_mod->edit($city_id, $userdata);
				$data['pdf_data'] = $this->Account_mod->Billing_details($city_id);
				generate_kyi_invoice_pdf($data);
				set_flashdata('success', 'Billing updated successfully');
				redirect('/admin/billing/listing');
            }
        }
		$data['result'] = $this->Account_mod->view($city_id);    

		$data['quality']	=	$this->Account_mod->quality_details();
		$data['purchaser']	=	$this->Account_mod->purchaser_details();
		$data['site']		=	$this->Account_mod->site_details();
		$data['seller']		=	$this->Account_mod->seller_details();
		$data['sgst']		=	$this->Account_mod->sgst();
	...
	public function view($id =null){
		...
		$data['users']= $this->Account_mod->Billing_details($id);
		$this->load->view('layout', $data);
	...
	public function view_all() {
        ...
        $query          =   $this->Account_mod->count_Billing_data();
       ...
       // $d = array('id' => 'city_id', 'name' => 'name', 'status' => 'status');
        $citydata = $this->Account_mod->get_Billing_data(); 
	...
	public function gen(){
		// pr($this->Account_mod->Billing_details('2')); die;
