...
class Setting_mod extends CI_Model {
    ...
    function edit($id = null, $userdata){  
        ...
        $this->db->where('id',$id);
        $this->db->update('aa_rokad',$userdata);
	...
	function Billing_details($id){
		...
		$this->db->join('aa_rokad as p_name', 'p_name.billing_id=ab.purchaser_name','left');
	...
	function count_Billing_data() {
        ...
        if (isset($_GET['status'])) {
            ...
            $this->db->where("aa_rokad.status =",$_GET["status"]);
        }
		
        if (!empty($requestData['search']['value'])) {
            $search_val = $requestData['search']['value'];
            $this->db->like("(CONCAT(aa_rokad.truck_no))", $search_val); 
        }
		
       
		return $query = $this->db->get('aa_rokad');
    }
...
function get_all_financial_year(){
  ...
  /**
    ...
    function checkRandomEntery(){
        ...
        $query = $this->db->get('aa_rokad')->row_array();
       // pr($query); die;
        if($query['billing_id'] == ''){
            return '1';
        }else{
