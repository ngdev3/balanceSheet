 ...
 * @since		Version 1.0
 */
class AccountMapping_mod extends CI_Model {
	...
	function Campaign_details($id){
		...
		$this->db->join('users', 'am_campaign.assign_for=users.id','left');
    ...
    function get_campaign_data($parent_id = "") {  
        ...
        $this->db->join('users as u','u.id = am_campaign.assign_for','left');
        $this->db->from("am_campaign");
        if (isset($_GET['status'])) {
    ...
    //echo $this->db->last_query();die;
        ...
        function account_mapping(){
          //  pr($_POST);  die;
            ...
            $up['account_no'] = $isFoundAccountDetail[1];
            $up['status_rec'] = 'done';
           // pr($up);  
          //  $this->db->where('CenterName', $_POST['rokad_type']);
